...
import { Search, User, Phone, Calendar, ShoppingBag, Wrench, X, ChevronRight, Package, CreditCard, Edit2, Save, FileText, Mail, MapPin } from 'lucide-react';
...
export const Customers = () => {
  ...
  const handleSaveCustomer = async () => {
    ...
    try {
      ...
      setSelectedCustomer({
        ...selectedCustomer,
        customer: { ...selectedCustomer.customer, name: editName.trim(), phone: editPhone.trim() },
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">{customers.length} clients enregistrés</p>
        </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 text-sm text-slate-400 mt-1">
                    ...
                    <span className="flex items-center gap-1">
                      <Calendar size={14} />
                      Client depuis {formatDate(customer.createdAt)?.split(',')[0]}
                    </span>
                  </div>
                </div>
                
                <div className="text-right">
                  <p className="text-primary-400 font-bold">{formatPrice(customer.totalSpent)}</p>
      ...
      {(selectedCustomer || loadingHistory) && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => !loadingHistory && setSelectedCustomer(null)}>
          <div className="bg-dark-surface rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            {loadingHistory ? (
              <div className="flex items-center justify-center py-20">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary-500"></div>
              </div>
                ...
                <div className="p-6 border-b border-dark-border">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h2 className="text-xl font-bold text-white">{selectedCustomer.customer.name}</h2>
                        <p className="text-slate-400">{selectedCustomer.customer.phone}</p>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-4 mt-6">
                    <div className="bg-dark-bg rounded-xl p-4 text-center">
                      <p className="text-2xl font-bold text-white">{formatPrice(selectedCustomer.customer.totalSpent)}</p>
                      <p className="text-sm text-slate-400">Total dépensé</p>
                    </div>
                    <div className="bg-dark-bg rounded-xl p-4 text-center">
                      <p className="text-2xl font-bold text-primary-400">{selectedCustomer.sales.length}</p>
                      <p className="text-sm text-slate-400">Achats</p>
                    </div>
                    <div className="bg-dark-bg rounded-xl p-4 text-center">
                      <p className="text-2xl font-bold text-amber-400">{selectedCustomer.repairs.length}</p>
                    ...
                    <div className="space-y-6">
                      ...
                      {saveSuccess && (
                        <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4 flex items-center gap-3">
                          ...
                          <p className="text-emerald-400">Modifications enregistrées avec succès</p>
                        </div>
                      )}
                      
                      {/* Edit/View Mode Toggle */}
                      <div className="flex justify-between items-center">
                        <h3 className="text-lg font-semibold text-white">Informations du client</h3>
                        {!isEditing ? (
                          <button
                      ...
                      <div className="bg-dark-bg rounded-xl p-6 space-y-4">
                        <div className="grid md:grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            {isEditing ? (
                              ...
                              <p className="text-white text-lg py-3">{selectedCustomer.customer.name}</p>
                            )}
                          </div>
                          ...
                          <div>
                            ...
                            {isEditing ? (
                              ...
                              <p className="text-white text-lg py-3">{selectedCustomer.customer.phone}</p>
                            )}
                          </div>
                          ...
                          <div>
                            ...
                            {isEditing ? (
                              ...
                              <p className="text-slate-500 text-lg py-3">{editEmail || '—'}</p>
                            )}
                          </div>
                          ...
                          <div>
                            ...
                            {isEditing ? (
                              ...
                              <p className="text-slate-500 text-lg py-3">{editAddress || '—'}</p>
                            )}
                          </div>
                        </div>
                        ...
                        {/* Notes */}
                        <div>
                          <label className="block text-slate-400 text-sm mb-2">Notes internes</label>
                          {isEditing ? (
                            <textarea
                              value={editNotes}
                            ...
                            <p className="text-slate-500 py-3">{editNotes || 'Aucune note'}</p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="bg-dark-bg rounded-xl p-6">
                        <h4 className="text-white font-medium mb-4">Informations supplémentaires</h4>
                        <div className="grid md:grid-cols-2 gap-4 text-sm">
                          <div className="flex justify-between py-2 border-b border-dark-border">
                            ...
                            <span className="text-white">{formatDateShort(selectedCustomer.customer.createdAt)}</span>
                          </div>
                          <div className="flex justify-between py-2 border-b border-dark-border">
                            <span className="text-slate-400">Première visite</span>
                            <span className="text-white">{formatDateShort(selectedCustomer.customer.firstVisit)}</span>
                          </div>
                          <div className="flex justify-between py-2 border-b border-dark-border">
                            <span className="text-slate-400">Total dépensé</span>
                            <span className="text-primary-400 font-medium">{formatPrice(selectedCustomer.customer.totalSpent)}</span>
                          </div>
                          <div className="flex justify-between py-2 border-b border-dark-border">
                            <span className="text-slate-400">Nombre de visites</span>
                            <span className="text-white">{selectedCustomer.sales.length + selectedCustomer.repairs.length}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      {selectedCustomer.sales.length === 0 ? (
                        ...
                        selectedCustomer.sales.map((sale) => (
                          <div key={sale.id} className="bg-dark-bg rounded-xl p-4 border border-dark-border">
                            <div className="flex justify-between items-start mb-3">
                              <div>
                                <p className="text-white font-medium">Vente #{sale.id}</p>
                                <p className="text-slate-500 text-sm">{formatDate(sale.createdAt)}</p>
                              </div>
                              <div className="text-right">
                                <p className="text-primary-400 font-bold">{formatPrice(sale.total)}</p>
                            ...
                            <div className="space-y-2 border-t border-dark-border pt-3">
                              {sale.items.map((item) => (
                                <div key={item.id} className="flex items-center gap-3 text-sm">
                                  ...
                                  <span className="flex-1 text-slate-300">{item.productName || 'Produit inconnu'}</span>
                            ...
                            {sale.discount > 0 && (
                              <div className="flex justify-between text-sm mt-2 pt-2 border-t border-dark-border">
                                <span className="text-amber-400">Remise appliquée</span>
                                <span className="text-amber-400">-{formatPrice(sale.discount)}</span>
                              </div>
                            )}
                          </div>
                        ))
                    ...
                    <div className="space-y-4">
                      {selectedCustomer.repairs.length === 0 ? (
                        ...
                        selectedCustomer.repairs.map((repair) => (
                          <div key={repair.id} className="bg-dark-bg rounded-xl p-4 border border-dark-border">
                            <div className="flex justify-between items-start mb-3">
                              <div>
                                <p className="text-white font-medium">{repair.deviceBrand} {repair.deviceModel}</p>
                                <p className="text-slate-500 text-sm">{formatDate(repair.createdAt)}</p>
                              </div>
                              <span className={`px-3 py-1 rounded-full text-xs font-medium ${getRepairStatusColor(repair.status)}`}>
                                {getRepairStatusLabel(repair.status)}
                              </span>
                            </div>
                            
                            <p className="text-slate-400 text-sm mb-3">{repair.issueDescription}</p>
                            ...
                            <div className="flex justify-between items-center pt-3 border-t border-dark-border">
                              <div className="text-sm">
                                ...
                                <span className="text-white ml-2">{formatPrice(repair.estimatedCost)}</span>
                              </div>
                              {repair.finalCost !== null && (
                                <div className="text-sm">
                                  <span className="text-slate-500">Final:</span>
                                  <span className="text-primary-400 ml-2 font-medium">{formatPrice(repair.finalCost)}</span>
                                </div>
                              )}
                            </div>
                          </div>
